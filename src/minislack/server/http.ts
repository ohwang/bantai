...
import { buildInteractive } from "./envelope"
import type { WebBundle } from "./web-bundle"
import type { WsRegistry } from "./ws-registry"
import type { BlockActionsPayload } from "../types/interactive"
...
export interface HttpContext {
  ...
  web?: WebBundle
  /**
   * WS-registry handle — used by /_minislack/block_action to dispatch a
   * synthetic `block_actions` envelope at the target app's live Socket
   * Mode connection. Optional because tests that don't drive
   * interactive payloads over HTTP can omit it.
   */
...
async function handleInternal(req: Request, pathname: string, ctx: HttpContext): Promise<Response> {
  try {
    ...
    if (pathname === "/_minislack/block_action" && req.method === "POST") {
      return handleBlockActionDispatch(req, ctx)
    }
    return new Response("not found", { status: 404 })
  } catch (err) {
...
/**
 * POST /_minislack/block_action — synthesise a Socket-Mode `block_actions`
 * envelope and deliver it to the target app. Lets external drivers (curl,
 * browsers) click Block Kit buttons on an already-running minislack +
 * bantai pair. Real Slack does this via an HTTPS callback or a Socket
 * Mode envelope generated by the Slack backend; either way the shape on
 * the receiving side is what Bolt dispatches as `action()` handlers.
 *
 * Minimum request body:
 *   { app_id, user_id, channel_id, message_ts, action_id }
 *
 * Optional fields: block_id, value, style ("primary"/"danger"), text.
 */
async function handleBlockActionDispatch(req: Request, ctx: HttpContext): Promise<Response> {
  if (!ctx.wsRegistry) return jsonErr("ws_registry_unavailable", 500)
  ...
  if (!body) return jsonErr("invalid_json", 400)
  const missing = (["app_id", "user_id", "channel_id", "message_ts", "action_id"] as const)
    .filter((k) => !body[k])
  if (missing.length > 0) {
    return jsonErr(`missing_fields:${missing.join(",")}`, 400)
  }
  ...
  if (!app) return jsonErr("app_not_found", 404)
  const user = ctx.ws.users.get(userId)
  if (!user) return jsonErr("user_not_found", 404)
  const channel = ctx.ws.channels.get(channelId)
  if (!channel) return jsonErr("channel_not_found", 404)
  const message = channel.messages.get(messageTs)
  if (!message) return jsonErr("message_not_found", 404)

  const payload: BlockActionsPayload = {
    type: "block_actions",
    team: {
      id: ctx.ws.team.id,
      domain: ctx.ws.team.domain,
    },
    user: {
      id: user.id,
      username: user.name,
      name: user.name,
      team_id: ctx.ws.team.id,
    },
    api_app_id: app.id,
    ...
    trigger_id: `${Date.now()}.${randomUUID()}`,
    channel: { id: channel.id, name: "name" in channel ? channel.name : channel.id },
    message: {
      type: "message",
      user: message.user,
      ts: message.ts,
    ...
    response_url: `${ctx.baseHttp()}/_minislack/response/${randomUUID()}`,
    actions: [
      {
        action_id: actionId,
        block_id: body.block_id ?? "minislack-synth-block",
        type: "button",
        text: body.text
          ? { type: "plain_text", text: body.text, emoji: true }
          : undefined,
        value: body.value,
        style: body.style,
        action_ts: `${(Date.now() / 1000).toFixed(6)}`,
      },
    ],
  ...
  const sent = ctx.wsRegistry.sendToApp(appId, envelope)
  if (sent === 0) return jsonErr("app_not_connected", 503)
  return jsonOk({ envelope_id: envelope.envelope_id, sent })
}
